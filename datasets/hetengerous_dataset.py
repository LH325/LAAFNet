...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        self.split = split  # train | train_aug | val.txt
...
from .sentinel2 import EuroSat
class Hetgenerousdataset(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(Hetgenerousdataset, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
        self.label_transform = label_transform
        # mean, meanb, std ,stdb = EuroSat.mean, EuroSat.mean_b,EuroSat.std, EuroSat.std_b
        #
        # self.transform =  EuroSat.build_transform(is_train, 512, mean, std)
        # self.transform2 = EuroSat.build_transform(is_train, 512, meanb, stdb)
    def __getitem__(self, index):
        name = self.img_name_list[index]
        A_path = get_img_path(self.root_dir, self.img_name_list[index % self.A_size])
        B_path = get_img_post_path(self.root_dir, self.img_name_list[index % self.A_size])
