...
def extract_high_frequency(image, radius_ratio=0.1):
    ...
    mask[:, center_w - radius_w:center_w + radius_w, center_h - radius_h:center_h + radius_h] = 0
...
class TransformerCBAMLayer(nn.Module):
    def __init__(self, channel1, channel2, reduction=16, spatial_kernel=7):
        ...
        self.cbam_conv = nn.Conv2d(4, 1, kernel_size=spatial_kernel, padding=spatial_kernel // 2, bias=False)
    ...
    def forward(self, x1, x2):
        ...
        min_out2 = torch.min(x2, dim=1, keepdim=True)
        spatial_out = self.sigmoid(self.cbam_conv(torch.cat([max_out1, avg_out1, max_out2, avg_out2, min_out1, min_out2], dim=1)))
...
class CGAFusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        pattn1 = sattn + cattn
        pattn2 = self.sigmoid(self.pa(initial))
...
class LAAFNet(nn.Module):
    def __init__(self, channel1, channel2, reduction=16, spatial_kernel=7):
        ...
        self.pixel_conv = nn.Conv2d(6, 6, kernel_size=spatial_kernel, padding=spatial_kernel // 2, bias=False)
        self.pixel_weight_conv = nn.Conv2d(6, 1, kernel_size=spatial_kernel, padding=spatial_kernel // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
        self._freeze_min_channels_grad()

    def _freeze_min_channels_grad(self):
        self.pixel_conv.weight.register_hook(self._zero_in_ch_4_5_grad_hook)
        self.pixel_weight_conv.weight.register_hook(self._zero_in_ch_4_5_grad_hook)

    @staticmethod
    def _zero_in_ch_4_5_grad_hook(grad: torch.Tensor):
        g = grad.clone()
        g[:, 4:6, ...] = 0
        return g

    @staticmethod
    def _masked_min_out_no_grad(x: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
        min_val, _ = torch.min(x, dim=1, keepdim=True)
        min_mask = (x <= (min_val + eps)).to(x.dtype)
        min_out = torch.sum(x * min_mask, dim=1, keepdim=True)
        return min_out.detach()
    ...
    def forward(self, x1, x2):
        ...
        max_out2, _ = torch.max(x2, dim=1, keepdim=True)
        avg_out2 = torch.mean(x2, dim=1, keepdim=True)
        min_out1 = self._masked_min_out_no_grad(x1)
        min_out2 = self._masked_min_out_no_grad(x2)
        spatial_in = torch.cat([max_out1, avg_out1, max_out2, avg_out2, min_out1, min_out2], dim=1)
        spatial_out = self.sigmoid(self.pixel_weight_conv(self.pixel_conv(spatial_in)))
        x = torch.cat([x1, x2], dim=1)
        x = x * spatial_out
        return x
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc, resnet_stages_num=5, backbone='resnet18', output_sigmoid=False, if_upsample_2x=True):
        ...
        if backbone == 'resnet18':
            self.resnet = models.resnet18(pretrained=False, replace_stride_with_dilation=[False, True, True])
        elif backbone == 'resnet34':
            self.resnet = models.resnet34(pretrained=True, replace_stride_with_dilation=[False, True, True])
        elif backbone == 'resnet50':
            self.resnet = models.resnet50(pretrained=False, replace_stride_with_dilation=[False, True, True])
            expand = 4
        else:
    ...
    def forward(self, x1, x2):
        ...
        xd = self.cbam(x1, x2)
        xm = F.interpolate(xm, size=(xl.size(2), xl.size(3)), mode='bilinear', align_corners=True)
...
class Base_EncoderDecoder(ResNet):
    def __init__(self, input_nc, output_nc, downsample_factor=8, in_channels=320, low_level_channels=24, num_classes=2, pretrained=False):
    ...
    def forward(self, x1, x2):
        ...
        co_deep1 = F.interpolate(x1, size=(x2.size(2), x2.size(3)), mode='bilinear', align_corners=True)
        ...
        middle_features = F.interpolate(middle_features, size=(low_level_features.size(2), low_level_features.size(3)), mode='bilinear', align_corners=True)
...
class VIT(ResNet):
    def __init__(self, input_nc, output_nc):
        ...
        self.vit1 = SETR(num_classes=64, image_size=256, patch_size=256 // 16, dim=1024, depth=24, heads=16, mlp_dim=2048, out_indices=(9, 14, 19, 23)).cuda()
        self.vit2 = SETR(num_classes=64, image_size=256, patch_size=256 // 16, dim=1024, depth=24, heads=16, mlp_dim=2048, out_indices=(9, 14, 19, 23)).cuda()
