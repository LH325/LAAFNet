...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        # self.gaussian_layer = GaussianConv2d(kernel_size=5, sigma=1, c=inp).to('cuda')
        self.channel=inp
        ...
        if expand_ratio == 1:
            ...
            self.conv = nn.Sequential(
                ...
                #   进行3x3的逐层卷积，进行跨特征点的特征提取
                #--------------------------------------------#
                nn.Conv2d(hidden_dim, hidden_dim, 3, stride, padding=3, dilation=3,groups=hidden_dim, bias=False),
                BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                #-----------------------------------#
                #   利用1x1卷积进行通道数的下降
    ...
    #
    #         return self.relu(self.IN(x + self.conv(x)))
    #     else:
    #         return self.conv(x)
